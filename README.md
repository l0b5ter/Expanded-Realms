...
To improve smoothness and prevent desync in multiplayer games, v0.0.2 added the "handshake" mechaninc. This to make sure player has the dlc and that everyone has the right toggles enabled. When the game start each player with the dlc will write their current dlc version to the game, on 1st turn the game will check version then sync toggles to the game host (player 0), meaning after this each player will have the same toggle state as game host.
